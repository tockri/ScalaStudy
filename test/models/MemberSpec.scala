  ...
  "Member" should {
    "create new record" in {
      DB.localTx { implicit session =>
        val now = new Date()
        val teamId = Bizteam.insert(new Bizteam(id = 0, name = "new team", createdAt = now))
        val team = Bizteam.get(teamId).value
        team.name mustEqual "new team"
        val memberId = Member.insert(new Member(id = 0, name = "taro", bizteamId = teamId, createdAt = now, team = team))
